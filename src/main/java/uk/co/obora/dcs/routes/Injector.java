...
import uk.co.obora.dcs.InboundPacket;
...
@Route("injector")
public class Injector extends VerticalLayout {
    ...
    @Autowired
    public Injector(DcsServerService service) {
        ...
        selectBar.add(serverSelect, createIcons());
    ...
    private HorizontalLayout createIcons() {
        ...
        // TODO: Move this to an AppLayout navigation bar/menu
        /*Icon cog = VaadinIcon.COG_O.create();
        cog.addClickListener(e -> {
            UI.getCurrent().navigate("server-editor");
        });*/
    ...
    private void executeCode() {
        try {
            InboundPacket response = uk.co.obora.dcs.Injector.doInjection(serverSelect.getValue(), code.getValue());

            Notification notification = Notification.show("Injection completed: " + response.getStatus() + "");
